...
<template>
  <div class="grid" :style="{ gridTemplateColumns: 'auto repeat(7, 1fr)' }">
    ...
    <div v-for="hour in hours" :key="hour" class="contents">
      ...
      <div v-for="(day, index) in weekDays" :key="index" class="border-b border-r h-[50px] px-1 relative">
        <!-- Contenu cellulaire -->

        <div v-for="reservation in props.allReservations" :key="reservation.id" class="h-full flex">
          <template v-if="isReservationInCell(day, hour, reservation)" class="">
            ...
            <template v-if="day.getDate() === new Date(parseInt(reservation.debut)).getDate() && hour === new Date(parseInt(reservation.debut)).getHours()">
              <div
                :class="{
                  'bg-blue-500/50 border-l-8 border-blue-500 text-white p-2  absolute inset-0 z-10 ': true,
                  'h-1/2 top-1/2': startsAtHalfHour(reservation),
                }"
              >
                {{ reservation.vehicules.immat }} {{ reservation.profiles.prenom }} {{ reservation.profiles.nom }}
              </div>
            </template>
            ...
            <template v-else-if="day.getDate() === new Date(parseInt(reservation.fin)).getDate() && hour === new Date(parseInt(reservation.fin)).getHours()">
              <div :class="endsAtHalfHour(reservation) ? 'bg-blue-500/50 border-l-8 border-blue-500 text-white p-2 absolute top-0 w-1/2  h-1/2 bottom-1/2' : 'invisible'">{{ reservation.vehicules.model }} - {{ reservation.profiles.prenom }} {{ reservation.profiles.nom }}</div>
            </template>
            ...
            <template v-else-if="(day > new Date(parseInt(reservation.debut)) || (day.getDate() === new Date(parseInt(reservation.debut)).getDate() && hour > new Date(parseInt(reservation.debut)).getHours())) && (day < new Date(parseInt(reservation.fin)) || (day.getDate() === new Date(parseInt(reservation.fin)).getDate() && hour < new Date(parseInt(reservation.fin)).getHours()))">
              <div class="bg-blue-500/50 border-l-8 border-blue-500 p-2 absolute top-0 inset-0"></div>
            </template>
          </template>
        </div>
      </div>
