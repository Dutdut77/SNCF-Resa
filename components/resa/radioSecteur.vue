...
<template>
  <div class="w-full h-full grid grid-cols-2 gap-4">
    <div class="relative w-full h-fit" v-for="(secteur, index) in secteurs" :key="index">
      ...
      <label :for="secteur.id" class="h-24 flex flex-col items-center justify-center rounded-lg bg-gradient-to-br from-white to-white border border-slate-300 hover:bg-opacity-75 peer-checked:shadow-lg peer-checked:shadow-sky-500/30 peer-checked:from-sky-600 peer-checked:to-sky-500 peer-checked:border-sky-500 peer-checked:text-white cursor-pointer transition">
