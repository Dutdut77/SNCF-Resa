...
<template>
  <section class="bg-slate-100 w-full h-full text-gray-600 pb-20 flex flex-col gap-4 overflow-auto">
    <div class="sticky h-16 px-4 flex items-center justify-between bg-slate-100">
      <div class="-space-y-1">
        <p class="text-medium text-xl">Anomalie</p>
    ...
    <div class="mt-auto mb-4 px-4 flex justify-between items-center">
      ...
      <AppButtonValidated v-if="etape == 4 && formValue.type == 0" class="w-32 ml-auto" theme="" @click="sendAnomalieSalle()"> <template #default> Envoyer </template> </AppButtonValidated>
      <AppButtonValidated v-if="etape == 4 && formValue.type == 1" class="w-32 ml-auto" theme="" @click="sendAnomalieVehicule()"> <template #default> Envoyer </template> </AppButtonValidated>
    </div>
  </section>
</template>
