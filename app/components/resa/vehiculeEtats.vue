...
onMounted(async () => {
  await getEtatsByVehicule(props.vehicule.id);
});
  ...
  <div class="w-full flex flex-col gap-4 px-4 pb-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-3 border-t border-slate-200">
      <div class="flex items-center gap-2">
        <h3 class="text-sm font-semibold text-slate-700 uppercase tracking-wider">État du véhicule</h3>
    ...
    <!-- Historique (en haut) -->
    <div v-if="etatsByVehicule.length === 0" class="text-center text-sm text-slate-400 py-6 border border-dashed border-slate-200 rounded-lg">
