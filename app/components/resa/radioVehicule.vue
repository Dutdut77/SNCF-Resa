  ...
  <div class="w-full h-fit grid grid-cols-1 xl:grid-cols-2 gap-2">
    <template v-if="dispoVehicules.length > 0">
      <div v-for="(vehicule, index) in dispoVehicules" :key="index" class="relative w-full">
        ...
        <label :for="vehicule.id" class="flex items-stretch rounded-xl border-2 border-slate-200 bg-white hover:border-sky-300 hover:shadow-sm peer-checked:border-sky-500 peer-checked:bg-sky-50/60 peer-checked:shadow-md peer-checked:shadow-sky-100/60 cursor-pointer transition-all duration-200 overflow-hidden">
          <!-- Immatriculation — bande verticale gauche -->
          <div class="flex items-center justify-center bg-linear-to-br from-sky-500 to-sky-600 peer-checked:bg-sky-600 px-3 shrink-0 transition-colors duration-200">
            <span class="font-mono text-base font-bold tracking-widest text-white">{{ vehicule.immat }}</span>
          </div>

          <!-- Infos -->
          <div class="flex flex-col justify-between gap-1.5 p-3 flex-1 min-w-0">
            <!-- Marque / Modèle + badge -->
            <div class="flex items-start justify-between gap-2">
              ...
              <span v-if="vehicule.acces_restreint == 1" class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-semibold bg-amber-50 border border-amber-200 text-amber-600 shrink-0">
            ...
            <div class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500">
              ...
              <div v-if="vehicule.id_carburant == 2" class="flex gap-1 items-center">
                <Icon name="material-symbols:local-gas-station" size="13" class="text-slate-400" />
                <span>Diesel</span>
              </div>
